...
class Settings(BaseSettings):
    ...
    app_name: str = Field(
        default="RPA Scraping System", description="Nome da aplicação"
    )
    debug: bool = Field(default=False, description="Modo debug (True para development)")
    ...
    database_pool_size: int = Field(
        default=5, description="Tamanho do pool de conexões"
    )
    database_max_overflow: int = Field(
        default=10, description="Número máximo de conexões extras"
    )

    # Configurações do RabbitMQ
    # PREENCHER: Configurar credenciais do RabbitMQ no .env
    rabbitmq_host: str = Field(
        default="PREENCHER_HOST", description="Host do servidor RabbitMQ"
    )
    rabbitmq_port: int = Field(default=5672, description="Porta do RabbitMQ")
    rabbitmq_user: str = Field(
        default="PREENCHER_USER", description="Usuário do RabbitMQ"
    )
    rabbitmq_password: str = Field(
        default="PREENCHER_PASSWORD", description="Senha do RabbitMQ"
    )
    rabbitmq_queue_hockey: str = Field(
        default="scraper_hockey_queue", description="Nome da fila para jobs de Hockey"
    )
    rabbitmq_queue_oscar: str = Field(
        default="scraper_oscar_queue", description="Nome da fila para jobs de Oscar"
    )

    # Configurações dos Scrapers
    selenium_headless: bool = Field(
        default=True, description="Executar Selenium sem interface gráfica"
    )
    selenium_timeout: int = Field(
        default=30, description="Timeout padrão para operações Selenium (segundos)"
    )
